...
def set_name(params):
    file_name = 'Data/Total_data'
    file_name += '_bert' if params['bert_tokens'] else '_normal'
    file_name += f"_{params['type_attention']}_{params['variance']}_{params['max_length']}"
    if params['decay']:
        file_name += f"_{params['method']}_{params['window']}_{params['alpha']}_{params['p_value']}"
    file_name += f"_{params['num_classes']}.pickle"
    return file_name
...
def get_annotated_data(params):
    ...
    for post_id, info in data.items():
        row = {'post_id': post_id, 'text': info['post_tokens'], 'rationales': info['rationales']}
        # copy each annotator’s id/label/target
        for i, ann in enumerate(info['annotators'], start=1):
            row[f'annotatorid{i}'] = ann['annotator_id']
            row[f'label{i}']         = ann['label']
            row[f'target{i}']        = ann['target']
        # majority‐vote final label
        labels = [row[f'label{i}'] for i in (1,2,3)]
        final = max(set(labels), key=labels.count)
        row['final_label'] = 'undecided' if labels.count(final)==1 else final
        records.append(row)
    return pd.DataFrame(records)


def get_training_data(df, params, tokenizer):
    post_ids, texts, atts, labs = [], [], [], []
    errors = confused = 0
    print("total_data", len(df))
    for _, row in tqdm(df.iterrows(), total=len(df)):
        if row['final_label']=='undecided':
            confused += 1
            continue
        tokens, masks = returnMask(row, params, tokenizer)
        att_vec = aggregate_attention(masks, row, params)
        post_ids.append(row['post_id'])
        texts.append(tokens)
        atts.append(att_vec)
        labs.append(row['final_label'])
    print("attention_error:", errors)
    print("no_majority:", confused)
